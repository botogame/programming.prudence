...
Автозапуск по итогу должен проходить по папкам 1.4. => 1.3. => 1.2. => 1.1. ( при этом автозапуск на модули 1.2. не применяется, их можно вызвать только  `⚙️ запустить модуль`, где переменные ввода будут из 1.4.).
