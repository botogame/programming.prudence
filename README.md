...
/📁site.ru/📄.htaccess <-- RewriteRule ^(.*)$ /index.php?path=$1 [NC,L,QSA]
          ...
          /📁.htaccess/📄index.php <-- процедурный код
                      ...
                      /📁index.php/📄основа.sql <-- sql запросы
                                  /📁основа.sql/📄.history
```
...
От понятия дублей кода придётся избавиться, чтобы не нарушить структуру вложенности папок. Если  где то нужен синхрон, то добавляем  два окна редактирования или больше, в зависимости от кол-ва зафиксированного кода для синхрона. Так можно будет создать что то уникальное для какого то раздела, и потом применить для других, сохранив связи. Для базы данных это возможность быстро скорректировать запрос под задачу, не обращаясь к громоздкому инструменту, но сделать завязку, к чему должны прийти.
