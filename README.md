...
🎁 ядро
   ...
   📷 результат с _get
   🎁 реакция
        🔐 {если} {результат с _get} {равен} "start"
               📷 текст для echo
                      🔐 {установить значение} "Привет мир!"
               ⚙️ вывести текст
        🔐 {если} {результат с _get} {равен} "exit"
               ⚙️ остановить скрипт
   🎁 остановка
        🔐 {установить значение} {результат с _get} "exit"
        📷 позиция конструкции
...
Последовательность:
1. запускается конструкция 🎁 ядро
...
Сделаем так, чтобы 📷 ресурсы и ⚙️ процессы уже были полностью проработаны редактором, где выбирая ⚙️ процессы подгружаем перед-после нужные 📷 ресурсы (перед - то что подгружаются в ⚙️ процесс, после - то что выгружается). 📷 Ресурсы добавить изменить название как то отдельно запретим.
