...
8. здесь срабатывает exit, но даже если его и не было, то повторного запуска бы конструкции `🎁 остановка` не последовало (но отработались бы все конструкции что были бы внутри)
