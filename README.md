...
Вначале будет первый проект `подготовка среды разработки` где задачей будет `настройка`, далее `подготовка среды разработки / навигация` и т.д., что подтянется. Возможно можно будет по пути зафиксировать галочками "можно | нельзя изменять"  другими задачами (проектами).
