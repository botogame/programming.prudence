...
Чтобы не путаться в подобных цифрах - `31.0.2.1.212312.12332131232` будет закреплять модификации названиями проектов (`магазин`, `магазин / корзина`, `пользователи`). Так мы сможем ввести загрузку на dev и prod только нужных модификаций. Единственно нужно обдумать как быть с теми конструкциями, которые были изменены по двум проектам (ввести табу на ведение двух проектов для смежных конструкций?).

Первым проектом у нас будет: `подготовка среды разработки`, далее `подготовка среды разработки / навигация` и т.д., что подтянется. Можно будет по пути зафиксировать галочками "можно | нельзя" изменять.
